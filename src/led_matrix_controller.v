...
//   4. Optimized Sensitivity -> Reaches all 4 outer edges comfortably at ~20°–25°
...
module LED_Matrix_Controller (
    ...
    // Step 1: Threshold = ±7 LSB (Release back to center = ±4 LSB)
    // Step 2: Threshold = ±13 LSB (Release = ±10 LSB)
    // Step 3 (Edge): Threshold = ±19 LSB (Release = ±16 LSB) -> Reaches border rows/cols!
    // -----------------------------------------------------------------------
    reg [2:0] target_r0;
    reg [2:0] target_c0;
